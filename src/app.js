...
// Every color used across the body plus every active addon, de-duplicated by hex, in first-seen order.
function collectUsedColorNames(bodyColors, activeAddons){
  const usedColors = [];
  const seenColorHexes = new Set();
  const addUsedColor = c => { if(!seenColorHexes.has(c.hex)){ seenColorHexes.add(c.hex); usedColors.push(c); } };
  bodyColors.forEach(addUsedColor);
  activeAddons.forEach(a => a.colors.forEach(addUsedColor));
  return formatColorNameList(usedColors.map(c=>c.name));
}
...
function buildPatternText(body){
  ...
  const colorList = collectUsedColorNames(colors, activeAddons);

  const lines = [];
  lines.push(buildPatternTitle(body).toUpperCase());
  lines.push('An amigurumi pattern designed by MagicBearCrochet');
  lines.push('');
  ...
  lines.push('Varies depending on the pieces you choose and how firmly you stuff it.');
  lines.push('');
  lines.push('ABBREVIATIONS');
  lines.push('sc — single crochet');
  lines.push('inc — increase (2 sc in the same stitch)');
  lines.push('dec — decrease (single crochet 2 stitches together)');
  lines.push('st(s) — stitch(es)');
  lines.push('rnd — round');
  lines.push('');
  lines.push(`© ${new Date().getFullYear()} MagicBearCrochet. All rights reserved. This pattern is for personal use only. You may not copy, share, resell, teach from, or redistribute this pattern in any form, digital or physical. You may sell finished items handmade from this pattern, provided you credit "MagicBearCrochet" as the pattern designer. This pattern may not be used to train any artificial intelligence model.`);
  lines.push('');
...
// block from the live Google Doc template — only the title and color list
// vary per pattern. Ask Iana to confirm this still matches
// https://docs.google.com/document/d/1PKd7RqloJpUoH6X8EOHiZsAVdhGeUOgJiWL2IlS6WRo/edit
// and adjust as needed.
function buildPatternDocHeaderHtml(title, colorList){
  return `
    ...
    <p style="font-family:Nunito, Arial, sans-serif;font-size:12pt;color:#6B6072;margin:0 0 20px 0;">An amigurumi pattern designed by MagicBearCrochet</p>
    ...
    <h2 style="font-family:Nunito, Arial, sans-serif;font-size:16px;font-weight:800;color:#2B2130;margin:18px 0 8px 0;">Yarn</h2>
    <p style="font-family:Nunito, Arial, sans-serif;font-size:12pt;color:#2B2130;margin:0 0 4px 0;">Beginner / Tube Yarn: Cuddle Craft Tube or Easy Peasy</p>
    <p style="font-family:Nunito, Arial, sans-serif;font-size:12pt;color:#2B2130;margin:0 0 20px 0;">Colors: <b>${escapeHtml(colorList)}</b></p>
    <h2 style="font-family:Nunito, Arial, sans-serif;font-size:16px;font-weight:800;color:#2B2130;margin:18px 0 8px 0;">Difficulty</h2>
    <p style="font-family:Nunito, Arial, sans-serif;font-size:12pt;color:#2B2130;margin:0 0 20px 0;">Beginner</p>
    <h2 style="font-family:Nunito, Arial, sans-serif;font-size:16px;font-weight:800;color:#2B2130;margin:18px 0 8px 0;">Finished size</h2>
    <p style="font-family:Nunito, Arial, sans-serif;font-size:12pt;color:#2B2130;margin:0 0 20px 0;">Varies depending on the pieces you choose and how firmly you stuff it.</p>
    ...
    <p style="font-family:Nunito, Arial, sans-serif;font-size:9pt;color:#9A8FA0;margin:0 0 20px 0;">© ${new Date().getFullYear()} MagicBearCrochet. All rights reserved. This pattern is for personal use only. You may not copy, share, resell, teach from, or redistribute this pattern in any form, digital or physical. You may sell finished items handmade from this pattern, provided you credit "MagicBearCrochet" as the pattern designer. This pattern may not be used to train any artificial intelligence model.</p>
  `;
}
...
function buildPatternDocPieceHtml(heading, makeCount, colors, segments){
  ...
  html += `<p style="font-family:Nunito, Arial, sans-serif;font-size:12pt;color:#2B2130;margin:0 0 10px 0;">🧶&nbsp;&nbsp;<b>With ${escapeHtml(colorNames)} yarn.</b></p>`;
  segments.forEach(seg=>{
    if(seg.kind==='table'){
      seg.rows.forEach(r=>{
...
function buildPatternDocHtml(body){
  ...
  const colorList = collectUsedColorNames(bodyColors, activeAddons);

  let html = '<div>';
  html += buildPatternDocHeaderHtml(buildPatternTitle(body), colorList);
